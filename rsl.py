...
st.set_page_config(page_title="2026 라운즈 프로모션",
...
def load_data():
    ...
    csv_url = sheet_url.replace(
        '/edit?usp=sharing', '/export?format=csv').replace('/edit', '/export?format=csv')
    ...
    if '사업자번호' in df.columns:
        ...
        df['사업자번호'] = df['사업자번호'].astype(str).str.replace(
...
with tabs[0]:
    ...
    user_input = st.text_input("🏢 사업자번호 입력", placeholder="안경원 사업자번호를 숫자만 입력해주세요 ",
    ...
    if st.button("조회하기", use_container_width=True):
        ...
        if user_input:
            ...
            if not result.empty:
                ...
                grade_df['rank'] = grade_df['26/03'].rank(
                    method='min', ascending=False)

                user_rank = int(
                    grade_df[grade_df['사업자번호'] == search_num]['rank'].values[0])

                target_to = TO_MAP.get(grade, 10)

                display_limit = LIMIT_MAP.get(grade, 25)

                grade_sorted = grade_df.sort_values(
                    by='26/03', ascending=False).reset_index(drop=True)
                ...
                percent = int((current_amt / target_amt) *
                              100) if target_amt > 0 else 100
                ...
                if user_rank <= target_to:
                    ...
                    st.success(
                        f"🏆 현재 **{grade} 등급 {user_rank}위** | **[달성 혜택 1]** 안정권")

                    st.markdown(
                        f"합격선 대비 **{current_amt - target_amt:,}원** 초과 달성 중입니다.")

                elif user_rank <= display_limit:

                    st.warning(
                        f"🎯 현재 **{grade} 등급 {user_rank}위** | **[달성 혜택 1]** 진입까지 **{target_amt - current_amt:,}원**")

                else:

                    st.error(
                        f"🚀 현재 **{grade} 등급 {user_rank}위** | **[달성 혜택 2]** 집중 공략 구간")
                ...
                with col1:
                    ...
                    st.markdown(
                        f'<div class="p-card"><div class="p-label">위탁 등급</div><div class="p-value">{grade}</div></div>', unsafe_allow_html=True)
                ...
                with col2:
                    ...
                    st.markdown(
                        f'<div class="p-card"><div class="p-label">{label}</div><div class="p-value">{val:,}원</div></div>', unsafe_allow_html=True)
                ...
                with col3:
                    ...
                    st.markdown(
                        f'<div class="p-card"><div class="p-label">3월 발주액({update_time_str})</div><div class="p-value">{current_amt:,}원</div></div>', unsafe_allow_html=True)
                ...
                st.info(
                    "💡 지금 바로 상단의 **[🎁 달성 혜택 안내]** 탭을 클릭하여 우리 매장이 받을 수 있는 상세 혜택을 확인하세요!")
